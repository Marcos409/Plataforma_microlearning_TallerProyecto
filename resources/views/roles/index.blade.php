...
<div class="row mb-3">
    ...
    <div class="col-lg-2 col-md-4 col-sm-6 mb-3">
        <div class="card bg-{{ $role->name == 'Administrador' ? 'danger' : ($role->name == 'Docente' ? 'success' : 'info') }} text-white h-100">
            <div class="card-body text-center py-3">
                <i class="fas fa-{{ $role->name == 'Administrador' ? 'user-shield' : ($role->name == 'Docente' ? 'chalkboard-teacher' : 'user-graduate') }} fa-2x mb-2"></i>
                <h4 class="mb-1">{{ $role->users_count }}</h4>
                <p class="mb-0 small">{{ $role->name }}{{ $role->users_count != 1 ? 's' : '' }}</p>
            </div>
        </div>
    </div>
    @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadUsers(roleFilter = '') {
        ...
        fetch(`{{ route('admin.roles.users') }}?role=${roleFilter}`)
    ...
    function loadUsersForMassiveAssign() {
        fetch(`{{ route('admin.roles.users') }}?role=null`)
            .then(response => response.json())
            .then(data => {
    ...
    function assignMassiveRoles(userIds, roleId) {
        ...
        fetch('{{ route('admin.roles.assign.massive') }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
